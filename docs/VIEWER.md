...
| `POST /api/export/sidecars` | `[Edition]` Write sidecars for explicit paths or a filter set. Body `{paths?, filters?, exclude?, overwrite}` — either `paths` or a gallery `filters` set; `exclude` (optional, max 1000) drops named paths from a `filters` set; `filters` resolves through the same normalization `GET /api/photos` uses, exactly like `POST /api/cull/apply` below |
