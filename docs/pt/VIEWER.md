...
| `GET /api/photos/paths` | `{ total, paths }` — cada caminho correspondente, sem limite nem ordem; usado sob demanda por ações que precisam de nomes de arquivo (download, cópia), nunca por «Selecionar tudo» |
...
| `POST /api/export/sidecars` | `[Edition]` Grava sidecars para caminhos explícitos ou um conjunto de filtros. Corpo `{paths?, filters?, exclude?, overwrite}` — ou `paths`, ou um conjunto `filters` da galeria; `exclude` (opcional, máx. 1000) remove caminhos indicados de um conjunto `filters`; `filters` é resolvido pela mesma normalização que `GET /api/photos` usa, exatamente como `POST /api/cull/apply` abaixo |
