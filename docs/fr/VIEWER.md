...
| `GET /api/photos/paths` | `{ total, paths }` — tous les chemins correspondants, sans plafond ni tri ; utilisé à la demande par les actions qui ont besoin des noms de fichiers (téléchargement, copie), jamais par « Tout sélectionner » |
...
| `POST /api/export/sidecars` | `[Edition]` Écrire des sidecars pour des chemins explicites ou un ensemble de filtres. Corps `{paths?, filters?, exclude?, overwrite}` — soit `paths`, soit un ensemble `filters` de galerie ; `exclude` (optionnel, max 1000) retire des chemins nommés d'un ensemble `filters` ; `filters` est résolu via la même normalisation que celle utilisée par `GET /api/photos`, exactement comme `POST /api/cull/apply` ci-dessous |
