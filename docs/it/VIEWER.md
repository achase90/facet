...
| `GET /api/photos/paths` | `{ total, paths }` — ogni percorso corrispondente, senza limite né ordine; usato all'occorrenza dalle azioni che hanno bisogno dei nomi file (scaricamento, copia), mai da «Seleziona tutto» |
...
| `POST /api/export/sidecars` | `[Edition]` Scrivi i sidecar per percorsi espliciti o per un insieme di filtri. Corpo `{paths?, filters?, exclude?, overwrite}` — o `paths` oppure un insieme `filters` di galleria; `exclude` (opzionale, max 1000) rimuove percorsi indicati da un insieme `filters`; `filters` viene risolto tramite la stessa normalizzazione usata da `GET /api/photos`, esattamente come `POST /api/cull/apply` sotto |
